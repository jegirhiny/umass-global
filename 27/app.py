...
app.config['SQLALCHEMY_DATABASE_URI'] = (os.environ.get('DATABASE_URL', 'postgresql://postgres:password@localhost:5432/umass-global'))
...
def profile():
    ...
    if form.validate_on_submit():
        if form.password.data and not User.authenticate(g.user.username, form.password.data):
...
def delete_user():
    ...
    Message.query.filter_by(user_id=g.user.id).update({"user_id": None})
...
def liked_messages(user_id):
    """Show liked messages of the user."""

    user = User.query.get_or_404(user_id)
    liked_messages = user.likes

    return render_template('users/liked.html', user=user, messages=liked_messages)
